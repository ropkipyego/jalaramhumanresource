...
interface Run {
  ...
  employee?: { full_name: string; staff_id: string; kra_pin?: string; bank_name?: string; bank_branch?: string; bank_account?: string };
}
...
export default function PayrollPeriod() {
  ...
  const load = useCallback(async () => {
    ...
    const { data: r } = await supabase.from('payroll_runs')
      .select('*, employee:profiles!payroll_runs_employee_id_fkey(full_name, staff_id, kra_pin, bank_name, bank_branch, bank_account)')
  ...
  const totalGross = runs.reduce((sum, r) => sum + (Number(r.gross_earnings) || 0), 0);
        ...
        <CardContent>
          {runs.length === 0 ? (
            ...
            <Table>
              ...
              <TableBody>
                {runs.map((r) => (
                  <TableRow key={r.id} className="cursor-pointer" onClick={() => openPayslip(r)}>
